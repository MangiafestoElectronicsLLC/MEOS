...
Both add-ons ship in two build profiles: a Python 3 build for Kodi 19+/20+ and Firestick, and a separate Python 2 build for Kodi 18.7. The repository serves both automatically - Kodi picks the right one for your version - so you normally just install `repository.meos.zip` and let it update itself.

## Install (any Kodi device, local or global)

### A. Global / on-device browsing (no PC needed)
Use this when the device itself can browse the internet from Kodi's File Manager. This works on Kodi 18.7 (Leia) and Kodi 19+/20+ alike: the repository publishes two version-scoped feeds, so Leia receives the Python 2 builds and Matrix/Nexus/Omega receive the Python 3 builds.
...
3. This installs the MEOS repository, which auto-updates itself plus both `plugin.video.meos` and `MEOS Hub`.
...
Use this for devices that can't browse the GitHub Pages source, or for offline installs.
...
`scripts/build-kodi-repo.ps1` rebuilds every zip and both feeds, then runs `scripts/verify-repo-feed.ps1` and refuses to finish if the feed is inconsistent. Verification also runs in CI (`.github/workflows/verify-feed.yml`) on every push, covering:

- `addons.xml` / `addons-k18.xml` are LF-only, so git's CRLF normalization cannot invalidate the published `.md5`.
- Each `.md5` matches its feed byte-for-byte.
- Every advertised add-on has a downloadable zip at `<datadir>/<id>/<id>-<version>.zip`, with a matching version inside the zip.
- Each feed declares the right `xbmc.python` major version, and `addon.xml` keeps both version-scoped `<dir>` guards.

`scripts/check-live-feed.ps1` runs the same checksum and download checks against the published GitHub Pages and raw.githubusercontent hosts.
